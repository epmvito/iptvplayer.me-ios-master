...
<<<<<<< HEAD
            if let cookie = responseData.response?.value(forHTTPHeaderField: "set-cookie"), let login = parameters["login"], let password = parameters["pass"] {
                let userDefaults = UserDefaults.standard
                userDefaults.setValue(cookie, forKey: "set-cookie")
